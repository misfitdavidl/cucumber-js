function AstTreeWalker(features, supportCodeLibrary, listeners, options) {
  ...
  var self = {
          ...
          self.visitBeforeAllHooks(function() {
            if (lifecycleError) {
              ...
              features.acceptVisitor(self, function () {
                self.visitAfterAllHooks(function() {
                  self.visitFeaturesResult(callback);
                });
              });
            }
    ...
    visitBeforeAllHooks: function visitBeforeAllHooks(callback) {
      ...
      self.visitLifecycleHooks('Before All', beforeAllHooks, callback);
    },

    visitAfterAllHooks: function visitAfterAllHooks(callback) {
      var afterAllHooks = supportCodeLibrary.getAfterAllHooks();
      self.visitLifecycleHooks('After All', afterAllHooks, callback);
    },

    visitLifecycleHooks: function visitLifecycleHooks (keyword, hooks, callback) {
      var defaultTimeout = self.getDefaultTimeout();
      Cucumber.Util.asyncForEach(hooks, function (hook, callback) {
        if (lifecycleError) {
          callback();
        } else {
          var hookStep = Cucumber.Ast.HookStep(keyword);
          hook.invoke(null, null, null, defaultTimeout, function (stepResult) {
            if (stepResult.getStatus() !== Cucumber.Status.PASSED) {
              lifecycleError = true;
            }
