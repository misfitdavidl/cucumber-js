/* jshint -W106 */
function JsonFormatter(suite, log) {
  ...
  function formatTags(tags, parentTags) {
    var tagsProperties = [];
  ...
  suite.on('Background', function (background) {
    ...
    steps.forEach(function (value) { formatStep(value); });
  });
  ...
  suite.on('BeforeScenario', function (scenario) {
    ...
    if (tags.length > 0) {
      var formattedTags = formatTags(tags, parentFeatureTags);
